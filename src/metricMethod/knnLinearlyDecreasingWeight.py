...
# Определяем функцию линейно убывающего веса
def linearlyDecreasingWeight(distance):

  weight = np.ones_like(distance)
  endWeight = weight.shape[1]


  for i in range(endWeight):
    weight[:, i] *= (endWeight-i)/endWeight


  return weight


# Определяем модель классификатора KNN, где сосед K равен 5
# Задаём метрическое пространство и функцию изменения весов
neigh = KNeighborsClassifier(n_neighbors=5, metric="euclidean", weights=linearlyDecreasingWeight)
